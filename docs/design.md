...
Docker Pluginでは、セキュリティチェックは**タグ参照（例: `alpine:3.14`）に対してのみ**実行されます。ダイジェスト参照（例: `sha256:abc123...`）はフィルタリング対象外です。
...
1. **ダイジェストの不変性**: ダイジェストはコンテンツのハッシュ値であり、タグとは異なり特定のバージョンを一意に識別します。タグ `3.14` が脆弱だとしても、同じイメージを指すダイジェストを事前に知ることは困難です。

2. **Registry API の制約**: Docker Registry API v2 では、ダイジェストからタグへの逆引きは標準でサポートされていません。全タグを列挙して各マニフェストのダイジェストを取得する必要があり、パフォーマンス上現実的ではありません。

3. **ユースケースの違い**: ダイジェストは通常、CI/CD パイプラインやロックファイルで固定バージョンとして使用されます。これらは意図的な固定であり、タグベースの動的な参照とは性質が異なります。

**セキュリティ上の注意事項**:

> ⚠️ **重要**: タグベースのフィルタリングは、ダイジェストによる直接アクセスを防止しません。悪意あるユーザーがブロックされたイメージのダイジェストを知っている場合、ダイジェスト指定でプルできます。完全なブロックが必要な場合は、ネットワークレベルでの制御を検討してください。
