...
**受入基準**:
- [x] `DockerPlugin`構造体が定義されている
- [x] `RegistryPlugin`トレイトが実装されている
- [x] `/v2/{name}/manifests/{reference}`がハンドルできる
...
**受入基準**:
- [x] `build_router()`関数が実装されている
- [x] `/health`エンドポイントが定義されている
- [x] `/metrics`エンドポイントが定義されている
- [x] `/pypi/*`, `/go/*`, `/cargo/*`, `/v2/*`がルーティングされている
- [x] `/ui/*`, `/api/*`がルーティングされている
- [x] テストが4つ以上存在する（14個）
- [x] `cargo test router`が通過する
...
**受入基準**:
- [x] `AuthMiddleware`が実装されている（`auth_middleware`関数と`AuthLayer`）
- [x] `LoggingMiddleware`が実装されている（`LoggingLayer`と`logging_middleware`）
- [x] `TracingMiddleware`が実装されている（`TracingLayer`と`tracing_middleware`）
- [x] 認証スキップパスが設定可能（`AuthLayer::skip_path()`）
- [x] テストが3つ以上存在する（10個）
- [x] `cargo test middleware`が通過する
...
**受入基準**:
- [x] `Server`構造体が定義されている
- [x] `Server::new(config)`が実装されている
- [x] `Server::run(shutdown)`が実装されている
- [x] グレースフルシャットダウンが実装されている
- [x] テストが2つ以上存在する（4個）
- [x] `cargo test server`が通過する
...
**受入基準**:
- [x] プロキシハンドラー関数が実装されている（`registry_proxy_handler`）
- [x] セキュリティプラグインが順番にチェックされる（`AppState.security_plugins`で統合）
- [x] キャッシュヒット時はキャッシュから返却される（`AppState.cache_plugin`で統合）
- [x] キャッシュミス時はアップストリームに転送される（RegistryPluginで処理）
- [x] ブロック対象は403で返却される（プロキシルートで処理）
- [x] テストが4つ以上存在する（4個のプロキシルートテスト）
- [x] `cargo test proxy`が通過する

**依存関係**: タスク8.1, タスク4.2-4.5, タスク5.2
**推定工数**: 40分
**ステータス**: `DONE`

**備考**: 現在はスケルトン実装（ルーティングと基盤のみ）。セキュリティチェック・キャッシュ・アップストリーム転送のロジックは`AppState`を通じて統合されており、フェーズ11の統合タスクで完全に接続される予定。
